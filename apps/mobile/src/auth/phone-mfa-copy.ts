...
export const getPhoneMfaSignInCopy = (mode: PhoneMfaMode): PhoneMfaSignInCopy =>
  mode === 'enforce'
    ? {
        ...
        trustText:
          'Your password is managed by Supabase and never stored by JagrukSetu. A phone OTP is required before private complaints and profile data become available.',
      }
    : {
        description:
          'Use your email and password. Phone verification is optional while secure SMS delivery is being configured.',
        trustText:
          'Your password is managed by Supabase and never stored by JagrukSetu. Phone verification is being introduced in stages and is not required in this environment yet.',
      };
