...
-- Migrations in this part: 32
-- Part source range: 20260714130000_phase_6_communication_and_notification_schema.sql through 20260724120000_verified_civic_area_office_contacts.sql
--
...
begin
  ...
  if current_cutoff >= 49 then
    raise notice 'Skipping already-complete migration 20260723100000_password_change_audit_event.sql';
    return;
  end if;
  ...
  if current_cutoff <> 48 then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_GAP: 20260723100000_password_change_audit_event.sql';
  end if;

  execute $migration_20260723100000_password_change_audit_event$
...
$migration_20260723100000_password_change_audit_event$;

  if not (coalesce(pg_catalog.position('password_changed' in (
        select pg_catalog.pg_get_constraintdef(constraint_record.oid)
        from pg_catalog.pg_constraint as constraint_record
        inner join pg_catalog.pg_class as relation_record
          on relation_record.oid = constraint_record.conrelid
    ...
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_POSTCONDITION_FAILED: 20260723100000_password_change_audit_event.sql';
  end if;

  update local_wellness_bundle_state
  set
    cutoff_position = 49,
    cutoff_name = '20260723100000_password_change_audit_event.sql'
  where singleton;

  raise notice 'Applied migration 20260723100000_password_change_audit_event.sql';
end;
$guard_49$;
-- ============================================================================
...
begin
  ...
  if current_cutoff >= 50 then
    raise notice 'Skipping already-complete migration 20260723110000_prune_deferred_v1_subsystems.sql';
    return;
  end if;
  ...
  if current_cutoff <> 49 then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_GAP: 20260723110000_prune_deferred_v1_subsystems.sql';
  end if;

  execute $migration_20260723110000_prune_deferred_v1_subsystems$
...
$migration_20260723110000_prune_deferred_v1_subsystems$;
  ...
  if not (pg_temp.local_wellness_function_exists('private', 'v1_deferred_subsystems_pruned')
      ...
      and not pg_temp.local_wellness_relation_exists('complaints.complaint_comments')
      and pg_temp.local_wellness_function_exists('governance', 'resolve_complaint_contact_readiness')
      and pg_temp.local_wellness_function_exists('complaints', 'assignment_delivery_readiness')) then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_POSTCONDITION_FAILED: 20260723110000_prune_deferred_v1_subsystems.sql';
  end if;

  update local_wellness_bundle_state
  set
    cutoff_position = 50,
    cutoff_name = '20260723110000_prune_deferred_v1_subsystems.sql'
  where singleton;

  raise notice 'Applied migration 20260723110000_prune_deferred_v1_subsystems.sql';
end;
$guard_50$;
-- ============================================================================
...
begin
  ...
  if current_cutoff >= 51 then
    raise notice 'Skipping already-complete migration 20260723120000_jagruksetu_complaint_taxonomy.sql';
    return;
  end if;
  ...
  if current_cutoff <> 50 then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_GAP: 20260723120000_jagruksetu_complaint_taxonomy.sql';
  end if;

  execute $migration_20260723120000_jagruksetu_complaint_taxonomy$
...
$migration_20260723120000_jagruksetu_complaint_taxonomy$;

  if not (pg_temp.local_wellness_column_exists('routing', 'issue_categories', 'category_purpose')
      and pg_temp.local_wellness_column_exists('routing', 'issue_categories', 'taxonomy_code')
      and pg_temp.local_wellness_column_exists('routing', 'issue_categories', 'workflow_type')
      and pg_temp.local_wellness_column_exists('routing', 'issue_categories', 'sensitivity_class')
      and pg_temp.local_wellness_column_exists('routing', 'issue_categories', 'routing_profile_category_id')
      ...
      and pg_temp.local_wellness_function_exists('complaints', 'assert_taxonomy_selection')
      and pg_temp.local_wellness_trigger_exists('complaints', 'complaint_drafts', 'complaint_drafts_validate_taxonomy_selection')
      and pg_temp.local_wellness_trigger_exists('complaints', 'complaints', 'complaints_validate_taxonomy_on_submission')) then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_POSTCONDITION_FAILED: 20260723120000_jagruksetu_complaint_taxonomy.sql';
  end if;

  update local_wellness_bundle_state
  set
    cutoff_position = 51,
    cutoff_name = '20260723120000_jagruksetu_complaint_taxonomy.sql'
  where singleton;

  raise notice 'Applied migration 20260723120000_jagruksetu_complaint_taxonomy.sql';
end;
$guard_51$;
-- ============================================================================
...
begin
  ...
  if current_cutoff >= 52 then
    raise notice 'Skipping already-complete migration 20260723130000_citizen_phone_verification_without_mfa.sql';
    return;
  end if;
  ...
  if current_cutoff <> 51 then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_GAP: 20260723130000_citizen_phone_verification_without_mfa.sql';
  end if;

  execute $migration_20260723130000_citizen_phone_verification_without_mfa$
...
$migration_20260723130000_citizen_phone_verification_without_mfa$;

  if not (pg_temp.local_wellness_function_exists('public', 'user_has_verified_phone')
      and pg_temp.local_wellness_function_execute_privilege('service_role', 'public.user_has_verified_phone(uuid)')) then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_POSTCONDITION_FAILED: 20260723130000_citizen_phone_verification_without_mfa.sql';
  end if;

  update local_wellness_bundle_state
  set
    cutoff_position = 52,
    cutoff_name = '20260723130000_citizen_phone_verification_without_mfa.sql'
  where singleton;

  raise notice 'Applied migration 20260723130000_citizen_phone_verification_without_mfa.sql';
end;
$guard_52$;
-- ============================================================================
...
begin
  ...
  if current_cutoff >= 53 then
    raise notice 'Skipping already-complete migration 20260724100000_require_email_identity_for_auth_signup.sql';
    return;
  end if;
  ...
  if current_cutoff <> 52 then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_GAP: 20260724100000_require_email_identity_for_auth_signup.sql';
  end if;

  execute $migration_20260724100000_require_email_identity_for_auth_signup$
...
$migration_20260724100000_require_email_identity_for_auth_signup$;

  if not (pg_temp.local_wellness_function_exists('public', 'hook_require_email_identity')
      and pg_temp.local_wellness_function_execute_privilege('supabase_auth_admin', 'public.hook_require_email_identity(jsonb)')) then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_POSTCONDITION_FAILED: 20260724100000_require_email_identity_for_auth_signup.sql';
  end if;

  update local_wellness_bundle_state
  set
    cutoff_position = 53,
    cutoff_name = '20260724100000_require_email_identity_for_auth_signup.sql'
  where singleton;

  raise notice 'Applied migration 20260724100000_require_email_identity_for_auth_signup.sql';
end;
$guard_53$;
-- ============================================================================
...
begin
  ...
  if current_cutoff >= 54 then
    raise notice 'Skipping already-complete migration 20260724110000_v1_bmc_general_intake_and_handoffs.sql';
    return;
  end if;
  ...
  if current_cutoff <> 53 then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_GAP: 20260724110000_v1_bmc_general_intake_and_handoffs.sql';
  end if;

  execute $migration_20260724110000_v1_bmc_general_intake_and_handoffs$
...
$migration_20260724110000_v1_bmc_general_intake_and_handoffs$;

  if not (pg_temp.local_wellness_relation_exists('routing.complaint_handoff_actions')
      and pg_temp.local_wellness_forced_rls('routing.complaint_handoff_actions')
      and coalesce(pg_catalog.position('protected_handoff' in (
        select pg_catalog.pg_get_constraintdef(constraint_record.oid)
        from pg_catalog.pg_constraint as constraint_record
      ...
      and pg_temp.local_wellness_function_exists('public', 'list_complaint_taxonomy')
      and pg_temp.local_wellness_function_exists('complaints', 'complaint_category_display_name')) then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_POSTCONDITION_FAILED: 20260724110000_v1_bmc_general_intake_and_handoffs.sql';
  end if;

  update local_wellness_bundle_state
  set
    cutoff_position = 54,
    cutoff_name = '20260724110000_v1_bmc_general_intake_and_handoffs.sql'
  where singleton;

  raise notice 'Applied migration 20260724110000_v1_bmc_general_intake_and_handoffs.sql';
end;
$guard_54$;
-- ============================================================================
...
begin
  ...
  if current_cutoff >= 55 then
    raise notice 'Skipping already-complete migration 20260724120000_verified_civic_area_office_contacts.sql';
    return;
  end if;
  ...
  if current_cutoff <> 54 then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_GAP: 20260724120000_verified_civic_area_office_contacts.sql';
  end if;

  execute $migration_20260724120000_verified_civic_area_office_contacts$
...
$migration_20260724120000_verified_civic_area_office_contacts$;

  if not (pg_temp.local_wellness_relation_exists('governance.offices_verified_civic_area_scope_idx')
      and pg_catalog.position('''offices''' in pg_catalog.pg_get_functiondef(pg_catalog.to_regprocedure('public.resolve_verified_governing_bodies(double precision,double precision,double precision,timestamp with time zone)'))) > 0
      and pg_catalog.position('limit 25' in pg_catalog.pg_get_functiondef(pg_catalog.to_regprocedure('public.resolve_verified_governing_bodies(double precision,double precision,double precision,timestamp with time zone)'))) > 0
      and pg_temp.local_wellness_function_execute_privilege('service_role', 'public.resolve_verified_governing_bodies(double precision,double precision,double precision,timestamp with time zone)')) then
    raise exception using
      errcode = 'P0001',
      message = 'LOCAL_WELLNESS_MIGRATION_POSTCONDITION_FAILED: 20260724120000_verified_civic_area_office_contacts.sql';
  end if;

  update local_wellness_bundle_state
  set
    cutoff_position = 55,
    cutoff_name = '20260724120000_verified_civic_area_office_contacts.sql'
  where singleton;

  raise notice 'Applied migration 20260724120000_verified_civic_area_office_contacts.sql';
end;
$guard_55$;
-- ============================================================================
